...
create_initial_config() {
...

  {
    ...
    if [[ "$BUILD_OS" == windows ]]; then
      ...
      if grep -q '^CONFIG_DRIVER_DATALINK=y' "$profile_config" && ! grep -q '^CONFIG_DRIVER_DATALINK_MACOS=y' "$profile_config"; then
        printf 'CONFIG_DRIVER_DATALINK_MACOS=y\n'
      fi
