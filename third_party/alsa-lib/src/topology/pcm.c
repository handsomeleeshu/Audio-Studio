		    ...
		    void *bin, size_t size)
{
	struct snd_soc_tplg_dai *dai;
	snd_tplg_obj_template_t t;
	struct snd_tplg_dai_template dt;
	struct snd_tplg_stream_caps_template caps[2], *cap;
	unsigned int i;
	int err;
...
next:
	...
	if (size < sizeof(*dai)) {
		snd_error(TOPOLOGY, "dai: small size %d", size);
		return -EINVAL;
	}
	if (sizeof(*dai) != dai->size) {
		snd_error(TOPOLOGY, "dai: unknown element size %d (expected %zd)",
				    dai->size, sizeof(*dai));
		return -EINVAL;
	}
	if (sizeof(*dai) + dai->priv.size > size) {
		snd_error(TOPOLOGY, "dai: wrong private data size %d", dai->priv.size);
		return -EINVAL;
	}

	tplg_log(tplg, 'D', pos, "dai: size %d private size %d",
		 dai->size, dai->priv.size);

	dt.dai_name = dai->dai_name;
	dt.dai_id = dai->dai_id;
	dt.playback = dai->playback;
	dt.capture = dai->capture;
	dt.flag_mask = dai->flag_mask;
	dt.flags = dai->flags;
	for (i = 0; i < 2; i++) {
		if (i == 0 && !dai->playback)
			continue;
		...
		cap = &caps[i];
		dt.caps[i] = cap;
		if (dai->caps[i].size != sizeof(dai->caps[0])) {
			snd_error(TOPOLOGY, "dai: unknown caps structure size %d",
					    dai->caps[i].size);
			return -EINVAL;
		}
		cap->name = dai->caps[i].name;
		cap->formats = dai->caps[i].formats;
		cap->rates = dai->caps[i].rates;
		cap->rate_min = dai->caps[i].rate_min;
		cap->rate_max = dai->caps[i].rate_max;
		cap->channels_min = dai->caps[i].channels_min;
		cap->channels_max = dai->caps[i].channels_max;
		cap->periods_min = dai->caps[i].periods_min;
		cap->periods_max = dai->caps[i].periods_max;
		cap->period_size_min = dai->caps[i].period_size_min;
		cap->period_size_max = dai->caps[i].period_size_max;
		cap->buffer_size_min = dai->caps[i].buffer_size_min;
		cap->buffer_size_max = dai->caps[i].buffer_size_max;
		cap->sig_bits = dai->caps[i].sig_bits;
	}
	dt.priv = &dai->priv;

	bin += sizeof(*dai) + dai->priv.size;
	size -= sizeof(*dai) + dai->priv.size;
	pos += sizeof(*dai) + dai->priv.size;
