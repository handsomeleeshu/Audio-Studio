...
LinuxHostSocket::LinuxHostSocket(int fd, SocketType type, bool connected) : type_(type), fd_(fd), connected_(connected) {}

LinuxHostSocket::~LinuxHostSocket() {
  close();
}
...
DriverResult LinuxHostSocket::open(const SocketConfig& config) {
  ...
  fd_ = ::socket(AF_INET, socket_type | SOCK_CLOEXEC, 0);
...
DriverResult LinuxHostSocket::accept(std::unique_ptr<ISocket>& client, uint32_t timeout_ms) {
  ...
  client = std::unique_ptr<ISocket>(new LinuxHostSocket(accepted_fd, type_, true));
  return DriverResult::success();
}
...
DriverResult LinuxHostSocket::send(const uint8_t* data, size_t size, size_t& sent, uint32_t timeout_ms) {
  ...
  const auto rc = ::send(fd_, data, size, MSG_NOSIGNAL);
...
DriverResult LinuxHostSocket::waitFor(short events, uint32_t timeout_ms) const {
  if (fd_ < 0) return DriverResult::unavailable("socket is not open");
  pollfd descriptor {};
  descriptor.fd = fd_;
