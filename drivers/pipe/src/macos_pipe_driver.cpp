...
class MacOsPipeDriverFactory final : public IPipeDriverFactory {
public:
  std::string name() const override { return "macos"; }
  std::unique_ptr<IPipeDriver> create() const override { return std::make_unique<MacOsPipeDriver>(); }
};

const bool kMacOsPipeDriverRegistered = [] {
  auto status = PipeDriverRegistry::instance().registerFactory(std::make_unique<MacOsPipeDriverFactory>());
  (void)status;
  return true;
}();
...
MacOsPipeStream::MacOsPipeStream(MacOsPipeDriver& driver, PipeType type) : driver_(driver), type_(type) {}

MacOsPipeStream::~MacOsPipeStream() {
  close();
}

DriverResult MacOsPipeStream::open(const PipeConfig& config) {
  close();
  type_ = config.type;
...
DriverResult MacOsPipeStream::read(void* buffer, size_t capacity, size_t& read_bytes, uint32_t timeout_ms) {
  read_bytes = 0;
...
DriverResult MacOsPipeStream::write(const void* data, size_t size, size_t& written_bytes, uint32_t timeout_ms) {
  written_bytes = 0;
...
DriverResult MacOsPipeStream::flush() {
  if (write_fd_ < 0) return DriverResult::unavailable("pipe is not open");
  return DriverResult::success();
}

void MacOsPipeStream::close() {
  if (read_fd_ >= 0) {
    ::close(read_fd_);
...
bool MacOsPipeStream::isOpen() const {
  return read_fd_ >= 0 || write_fd_ >= 0;
}

DriverResult MacOsPipeStream::waitFor(int fd, short events, uint32_t timeout_ms) const {
  pollfd descriptor {};
  descriptor.fd = fd;
...
std::unique_ptr<IPipeStream> MacOsPipeDriver::createPipeStream(PipeType type) {
  return std::make_unique<MacOsPipeStream>(*this, type);
}

DriverResult MacOsPipeDriver::createPipe(const PipeEndpoint& endpoint, PipeType type) {
  if (!isFifoType(type)) return DriverResult::invalidArgument("createPipe only supports FIFO/named pipe on macOS");
...
DriverResult MacOsPipeDriver::removePipe(const PipeEndpoint& endpoint, PipeType type) {
  if (!isFifoType(type)) return DriverResult::invalidArgument("removePipe only supports FIFO/named pipe on macOS");
  if (endpoint.path.empty()) return DriverResult::invalidArgument("pipe path is empty");
...
DriverResult MacOsPipeDriver::exists(const PipeEndpoint& endpoint, bool& result) {
  if (endpoint.path.empty()) return DriverResult::invalidArgument("pipe path is empty");
  struct stat statbuf {};
