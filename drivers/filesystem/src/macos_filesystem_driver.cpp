#include "macos_filesystem_driver.hpp"
...
class MacOsFileSystemDriverFactory final : public IFileSystemDriverFactory {
public:
  std::string name() const override { return "macos"; }
  std::unique_ptr<IFileSystemDriver> create() const override { return std::make_unique<MacOsFileSystemDriver>(); }
};

const bool kMacOsFileSystemDriverRegistered = [] {
  auto status = FileSystemDriverRegistry::instance().registerFactory(std::make_unique<MacOsFileSystemDriverFactory>());
  (void)status;
  return true;
}();
...
DriverResult MacOsFile::open(const std::string& path, const FileOpenOptions& options) {
  ...
  if (ec) return MacOsFileSystemDriver::filesystemError("resolve file path", path);

  const bool exists = std::filesystem::exists(absolute_path, ec);
  if (ec) return MacOsFileSystemDriver::filesystemError("check file", absolute_path.string());
  ...
  if (options.create && !exists) {
    ...
    if (!created) return MacOsFileSystemDriver::filesystemError("create file", absolute_path.string());
  }
  ...
  if (!stream_.is_open()) return MacOsFileSystemDriver::filesystemError("open file", absolute_path.string());
...
DriverResult MacOsFile::read(void* buffer, size_t capacity, size_t& read_bytes) {
  read_bytes = 0;
...
DriverResult MacOsFile::write(const void* data, size_t size, size_t& written_bytes) {
  written_bytes = 0;
  if (!stream_.is_open()) return DriverResult::unavailable("file is not open");
...
DriverResult MacOsFile::seek(uint64_t offset) {
  if (!stream_.is_open()) return DriverResult::unavailable("file is not open");
  stream_.clear();
...
DriverResult MacOsFile::tell(uint64_t& offset) {
  if (!stream_.is_open()) return DriverResult::unavailable("file is not open");
  offset = offset_;
  return DriverResult::success();
}

DriverResult MacOsFile::flush() {
  if (!stream_.is_open()) return DriverResult::unavailable("file is not open");
  stream_.flush();
...
std::unique_ptr<IFile> MacOsFileSystemDriver::createFile() {
  return std::make_unique<MacOsFile>();
}

DriverResult MacOsFileSystemDriver::exists(const std::string& path, bool& result) {
...
DriverResult MacOsFileSystemDriver::remove(const std::string& path) {
  if (path.empty()) return DriverResult::invalidArgument("path is empty");
  std::error_code ec;
...
DriverResult MacOsFileSystemDriver::rename(const std::string& from, const std::string& to) {
...
DriverResult MacOsFileSystemDriver::createDirectory(const std::string& path, bool recursive) {
...
DriverResult MacOsFileSystemDriver::listDirectory(const std::string& path, std::vector<FileInfo>& entries) {
  if (path.empty()) return DriverResult::invalidArgument("directory path is empty");
  std::error_code ec;
...
DriverResult MacOsFileSystemDriver::stat(const std::string& path, FileInfo& info) {
  if (path.empty()) return DriverResult::invalidArgument("path is empty");
  std::error_code ec;
...
std::string MacOsFileSystemDriver::joinPath(const std::vector<std::string>& parts) {
...
std::string MacOsFileSystemDriver::normalizePath(const std::string& path) {
  if (path.empty()) return {};
  return std::filesystem::path(path).lexically_normal().string();
}

std::string MacOsFileSystemDriver::absolutePath(const std::string& path) {
  std::error_code ec;
...
DriverResult MacOsFileSystemDriver::filesystemError(const std::string& operation, const std::string& path) {
