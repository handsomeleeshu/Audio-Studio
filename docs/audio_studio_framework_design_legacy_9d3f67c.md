...
过渡期实现原则是接口稳定、实现替换：`MockRuntimeEngine` 只作为 legacy smoke test 和非真实 runtime 的测试替身；pipeline build 这类需要真实产物的路径由 `BuildOrchestrator` 负责，并通过 `ConfigCompileClient`、`ValidationRunner` 这类 driver 接口逐步接入真实 `as_server` 和 simulator。
...
GUI/backend 可以继续保留 mock runtime 供 legacy smoke test 和非真实 runtime demo 使用；真实 build/run 能力应逐步通过可替换 driver 接入 as_server。
...
SOF 基础 component 的 bytes 型参数由 `plugins/builtin_module_configs` 打包为 SOF IPC3 payload；只有声明 `PIPE_RUNNING` 的 bytes 参数才生成标准 `SectionControlBytes` 并随 `.conf` 进入 `alsatplg` 编译出的 `.tplg`。未知或 Audio Studio 专用参数仍进入 `AS_PRIVATE`，由 A2 ASoC Codec 驱动按 private metadata 自动暴露自定义 KControl；标准 Linux + SOF 方案可忽略这些 A2 private bytes 控件，但仍能正确解析和创建 pipeline/PCM。
...
parameter.apply.settable_states:
  ...
  只有包含 PIPE_RUNNING 的 parameter，才允许由 as_config 例化成 ALSA KControl。
...
```text
只有 apply.settable_states 包含 PIPE_RUNNING 的 parameter，才生成 ALSA KControl。
不包含 PIPE_RUNNING 的 parameter，只进入 A2 private install/config/preset block。
```
...
3. 建立 module_type symbol table。
4. 展开 pipeline 的 node、edge 和 endpoint params。
...
| `resource_catalog.compute.cores` | 不生成标准 ALSA 对象 | 必须保存 | DSP/core affinity 和服务核信息 |
| `imports` | 不生成标准 ALSA 对象 | 不写入 | 导入共享 module catalog，例如 `configs/built-in-algorithm.json` |
| `module_types` | 可生成 widget/control 模板 | 必须保存 | module type、IO、parameter 模板；重复 `type_id` 是错误 |
| `pipelines[].nodes` | DAPM widgets、HOST PCM、DAI/BE widgets | 必须保存 | 每个 node 直接声明 `module_type` 和初始化 `params` |
| `pipelines[].edges` | DAPM routes | 必须保存 | derive.dapm=auto 时自动生成 |
| `frontend_connections` | 不生成标准 ALSA 对象 | 不写入 tplg private | GUI/runtime metadata；结构同 pipeline 的 `nodes[]/edges[]`，as_config 编译忽略 |
...
#### 7. resource_catalog 映射

当前 schema 只保留编译真正需要的 `resource_catalog.compute.cores[]`。它不是完整硬件初始化描述，不替代 DTS/ACPI，只用于 as_config 生成 scheduler/core affinity hint 和 Audio Studio private metadata。HOST/DAI 的 stream、link、format、tdm slot 等端点信息直接写在对应 pipeline node 的 `params` 中。
...
所有 node `params` key 都必须在对应 module type 的 `parameters[]` 中声明。三方算法仍通过 `Audio-Studio/plugins` 提供 module catalog 与 module config handler，不需要理解 HOST/DAI 特例。

---

#### 9. pipelines / frontend_connections 映射

`pipelines[]` 是唯一进入 as_config/tplg 编译的 graph。每个 node 形如：

```json
{
  "node_id": "VOL",
  "name": "Vol Playback",
  "module_type": "gain.volume",
  "params": {"gain_db": -3.0}
}
```

HOST 和 DAI 也使用同样 node 结构。`builtin.host` 通过 `stream_name`、`direction`、`channels_min/max`、`sample_bits`、`sample_rates` 描述 HOST stream；`builtin.dai` 通过 `dai_type`、`dai_index`、`direction`、`link_name`、`device_id`、`sample_rate`、`channels`、`sample_bits`、`tdm_slots`、`slot_width` 描述 DAI/BE。pipeline 不再有全局 `format` 或 `endpoint rate`，采样率从 endpoint node params 推导。

`frontend_connections[]` 与 `pipelines[]` 并列，使用同样的 `nodes[]` / `edges[]` 形状持久化前端 File Input/Output 节点和连接关系。例如：

```json
{
  "pipeline_id": "PLAYBACK_MAIN",
  "nodes": [
    {"node_id": "FILE_IN", "name": "File Input Playback", "module_type": "builtin.file_input", "params": {"enable": true, "file_path": "", "loop": true}, "ui": {"dx": -185, "dy": 0}}
  ],
  "edges": [
...
GUI/frontend 用 `pipelines[] + frontend_connections[]` 渲染 layout；backend build/save 同步更新这两个 section；`ConfigService` 编译忽略 `frontend_connections[]`。
...
bool a2_param_should_generate_kcontrol(const struct a2_param *p)
{
    ...
    if (!contains(p->apply.settable_states, "PIPE_RUNNING"))
        return false;
...
|---|---:|---|
| `["PIPE_UNLOADED"]` | 否 | build/load 前可配置，写入 node params/private config |
| `["PIPE_UNLOADED", "PIPE_LOADED"]` | 否 | build 后未运行时可配置，暂存到 Inspector preset |
| `["PIPE_UNLOADED", "PIPE_LOADED", "PIPE_RUNNING"]` | 是 | 运行中也可配置，预留 as_control/KControl 接入 |
| `["PIPE_RUNNING"]` | 是 | 仅运行中可配置 |
...
`Generate KControl=Yes` 表示该 parameter 在当前 `A2.json` 中包含 `PIPE_RUNNING` settable state。
实际 ID 不在 JSON 中保存，统一由 `as_config` 输出到 `include/as_config_ids.h` 和 `<project>_controls.csv`。

| 类别 | 当前数量 | 生成内容 |
| --- | ---: | --- |
| Runtime parameters | 8 | `control_uid`、ALSA KControl、runtime default config payload |
| Install-time parameters | 59 | `param_uid`、install config payload |
| Preset parameter groups | 3 | `preset_uid`、按 node 聚合的 config payload transaction |

---

#### 11. pipeline node 映射

独立 `node instances[]` 已取消。node 本身就是实例，`node_id` 是 pipeline 内稳定 id，`name` 是 UI/控制显示名，`module_type` 绑定 module catalog，`params` 保存该 node 的初始化参数。

| node 字段 | ALSA topology | Audio Studio private data |
|---|---|---|
| `node_id` | widget/control name 组成部分 | 必须保存 |
| `name` | KControl display name 组成部分 | 必须保存 |
| `module_type` | widget private type | 必须保存 |
| `params` | install-time config、endpoint config、runtime default | 必须保存并按 module type 校验 |
| `ui` | 不生成 | 不写入 tplg private | GUI layout hint，仅 frontend/backend 使用 |

HOST node 生成 PCM-facing widget；DAI node 生成 DAI/BE/hw_config；普通算法 node 生成 DSP component widget。KControl/private control id 始终使用 `pipe_id:node_id:module_type:param_id`，不再依赖 instance id。

---

#### 12. pipeline endpoint 生成规则

1. `builtin.host` 的 `direction=playback` 表示外部 File Input/host stream 进入 SOF，端口为 `in` external、`out` SOF；`direction=capture` 反向。
2. `builtin.dai` 的 `direction=playback` 作为 SOF sink，端口为 `in`；`direction=capture` 作为 SOF source，端口为 `out`。
3. PCM rate/channel/format 从 HOST/DAI node params 推导；DAI 侧 `sample_rate` 是硬件 fsync，HOST 侧可声明可接受的 `sample_rates` 列表。
4. `tdm_slots`、`slot_width`、`link_name` 保留行业通用命名，直接进入 DAI hw_config/private metadata。

---

#### 13. 当前 pipeline 列表

当前 `configs/platform/a2/A2.json` 与 `configs/platform/simulator/simulator.json` 覆盖三条 FILE_IO pipeline：

| pipe_id | domain | 说明 |
|---|---|---|
| `PLAYBACK_MAIN` | playback | `builtin.file_input` -> HOST -> playback algorithms -> FILE_IO DAI |
| `CAPTURE_MAIN` | capture | FILE_IO DAI -> capture algorithms/HOST -> `builtin.file_output` |
| `DSP_FILTER_COVERAGE` | playback | 覆盖 DSP filter module config |
...
| pipe_id | from | to |
| --- | --- | --- |
| PLAYBACK_MAIN | HOST_IN:out | CHREMAP:in |
| PLAYBACK_MAIN | CHREMAP:out | DELAY:in |
| PLAYBACK_MAIN | DELAY:out | FADER:in |
| PLAYBACK_MAIN | FADER:out | VOLUME:in |
| PLAYBACK_MAIN | VOLUME:out | DAI_OUT:in |
| CAPTURE_MAIN | DAI_IN:out | VOLUME:in |
| CAPTURE_MAIN | VOLUME:out | SRC:in |
| CAPTURE_MAIN | SRC:out | CHREMAP:in |
| CAPTURE_MAIN | CHREMAP:out | HOST_OUT:in |
| DSP_FILTER_COVERAGE | HOST_IN:out | DSP_FILTER:in |
| DSP_FILTER_COVERAGE | DSP_FILTER:out | DAI_OUT:in |
...
当前 `A2.json` 中，按 `PIPE_RUNNING` settable 规则会生成以下 KControl：


| pipe_id | node_id | inst_id | module_type | param_id | name | type | control_uid |
| --- | --- | --- | --- | --- | --- | --- | --- |
| PLAYBACK_MAIN | VOLUME | VOLUME | gain.volume | enable | Enable | bool | 0x9979FDF4 |
| PLAYBACK_MAIN | VOLUME | VOLUME | gain.volume | volume_db | Volume | float | 0x8E4B1FA4 |
| PLAYBACK_MAIN | VOLUME | VOLUME | gain.volume | mute | Mute | bool | 0xB83EB4F2 |
| CAPTURE_MAIN | VOLUME | VOLUME | gain.volume | enable | Enable | bool | 0x980BFC2F |
| CAPTURE_MAIN | VOLUME | VOLUME | gain.volume | volume_db | Volume | float | 0x71ACFE41 |
| CAPTURE_MAIN | VOLUME | VOLUME | gain.volume | mute | Mute | bool | 0xC711E259 |
| CAPTURE_MAIN | SRC | SRC | rate.src | enable | Enable | bool | 0x73E51DA6 |
| CAPTURE_MAIN | SRC | SRC | rate.src | dither | Dither | bool | 0xA66014DD |

##### 15.5 当前不会生成 KControl 的 install-time 参数

以下参数不包含 `PIPE_RUNNING` settable state，因此不生成普通 ALSA KControl，只进入 A2 private install/config block：


典型例子包括 `PLAYBACK_MAIN/CHREMAP.layout`、`PLAYBACK_MAIN/DELAY.max_delay_ms`、`PLAYBACK_MAIN/FADER.balance`、`DSP_FILTER_COVERAGE/DSP_FILTER.filter_preset` 以及 HOST/DAI 的 stream/link/format 参数。它们仍会写入 private install/config/preset payload，且 SOF component widget 与 `SOF_TKN_PROCESS_TYPE` 仍正常生成，只是不暴露为运行态 KControl。
...
2. on_install 参数不能在 PIPE_RUNNING 状态下通过普通 KControl 设置；
...
| --- | --- | --- | --- | --- | --- | --- |
| playback.music | Playback volume baseline | bulk | True | as_listed | True | 1 |
| capture.default | Capture volume baseline | bulk | True | as_listed | True | 1 |
| inspector_preset | GUI Inspector scratch values | inspector | True | as_listed | True | 2 |
...
| --- | --- | --- | --- | --- |
| playback.music | PLAYBACK_MAIN | VOLUME | volume_db | 0 |
| capture.default | CAPTURE_MAIN | VOLUME | volume_db | 0 |
| inspector_preset | PLAYBACK_MAIN | VOLUME | volume_db | 0 |
| inspector_preset | CAPTURE_MAIN | VOLUME | volume_db | 0 |
...
当前 `A2.json` 不声明 `scenes[]`。GUI build/run 使用 layout working group 和 pipeline runtime state；产品级 scene 后续如需暴露，应作为显式 schema 增量添加。
...
| node 引用不存在 module_type | `module_type=xxx` 未定义 |
| pipeline node 缺少或引用不存在 module type | `node.module_type=xxx` 未定义 |
| edge endpoint 不存在 | `VOL:foo` 的 node 或 port 未定义 |
...
| PIPE_RUNNING KControl 参数 type 未被 driver 支持 | strict 模式 |
...
| endpoint 参数不完整 | 无法推导 PCM/DAI 能力时 warning 或 fatal，取决于 strict 模式 |
    ...
    for_each_pipeline(pipe, ir) {
        for_each_node(node, pipe) {
            ...
            for_each_param(param, mt) {
                if (!contains(param->apply.settable_states, STUDIO_STATE_PIPE_RUNNING)) {
...
| DAI | 3 |
| KControl | 8 |
| install-time parameters | 59 |

生成的 PCM：

```text
A2 AS Config Playback
A2 AS Config Capture
A2 AS Config DSP Filter Coverage
```

生成的 DAI：

```text
FILE_IO_PLAYBACK_DAI0
FILE_IO_CAPTURE_DAI0
FILE_IO_DSP_FILTER_DAI1
```

生成的 pipeline graph：

```text
PLAYBACK_MAIN       : AS Config Playback
CAPTURE_MAIN        : AS Config Capture
DSP_FILTER_COVERAGE : AS Config DSP Filter Coverage
```
...
是否生成 KControl 只看 apply.settable_states 是否包含 PIPE_RUNNING；
...
Audio Studio GUI backend 不直接修改源平台 JSON。页面初始化加载平台配置时，backend 为当前 project 创建临时 workspace，将原始 JSON 拷贝为 `${platform}_pipeline_all.json`，并把该 all copy 返回前端。Build 时前端提交当前 layout snapshot，backend 按 working groups 重建 `${platform}_pipeline_all.json` 的根 `pipelines[]` 与 `frontend_connections[]`，并把 GUI snapshot 写入 `audio_studio_gui` 字段用于 UI 恢复和审计。

重生成规则：

```text
1. backend 遍历 snapshot.working_groups 重建当前 build 配置；单 pipeline snapshot 只生成该 pipeline。
2. 能唯一对应原始 pipeline 的 group 沿用原 pipe_id、domain、frame、runtime；新增/拆分/合并 group 使用稳定 GUI_PIPE_<index> 或前端提供的 pipeline_id。
3. SOF 节点直接写入 { node_id, name, module_type, params }。
4. File Input/Output 节点和 external/debug 连接写入 frontend_connections[]，不进入 pipelines[]。
5. as_config 只读取 pipelines[]、module_types[]、presets[] 等编译语义字段；frontend_connections[] 只供 GUI/backend runtime 使用。
```
...
编译成功后，backend 生成 `audio_studio_test_list.txt`，内容包含 `ac_run --endpoint as_datalink --mtu 512`、`trace on`、`pipeinstall <compiled.tplg>`。验证 runner 通过 `application/rv32qemu/sof-build-test.py` 启动 simulator 和验证用 as_server；ready marker 到达后 backend 返回 `runtime_state:"PIPE_LOADED"`。build 成功响应携带 `updated_pipelines`、`updated_frontend_connections` 与 `workspace_revision`，frontend 用这些字段同步内存中的当前 workspace config。

每次 build 开始前都会停止已有 validation session，因此 build 总是重新启动 simulator 和验证用 as_server。Frontend Build 始终提交 `group_id:"ALL"` 并一次性编译 layout 中所有 working groups；单 pipeline 选择只影响 RUN/STOP。`POST /api/pipeline/unload` 是全局 unload，会停止当前 session 并把 frontend/backend runtime state 退回 `PIPE_UNLOADED`。GUI `Save` 只有最近一次 build 成功后才允许把 `${platform}_pipeline_all.json` 写回原始平台 JSON；写回前 backend 会 strip `audio_studio_gui`，临时 workspace 仍保留该 GUI-only section。Build 成功后 frontend 只锁定结构编辑；拖动节点位置、Auto Arrange、选择、缩放和 Inspector 查看仍然允许。
...
`NOT_READY` 与 `ERROR` 是 UI 内部不可用/错误态，不作为参数 settable state 使用。平台 JSON 中 `parameter.apply.settable_states` 只使用 `PIPE_UNLOADED`、`PIPE_LOADED`、`PIPE_RUNNING` 三态，Inspector 按该列表自动置灰。`PIPE_UNLOADED` 表示 build/load 前可改，`PIPE_LOADED` 表示 build 后未运行时可改，`PIPE_RUNNING` 表示运行中可改。Inspector 展示参数时，必须先查找 `inspector_preset`；不存在则创建空 preset。`inspector_preset` 没有对应 pipeline/node/parameter 值时，显示 module parameter default。运行中修改会调用 `/api/param/update`，backend 更新 workspace `inspector_preset` 并返回 `control_apply:"pending_as_control"`，后续接入 as_control 后再真正下发 DSP 控制。

#### 5.X.3 HOST/DAI endpoint module schema

平台 JSON 不再在 pipeline 中声明独立端点表。HOST 与 DAI 都是普通 node，写在 `pipelines[].nodes[]` 中：
...
`builtin.file_input` 和 `builtin.file_output` 是前端 runtime 节点，持久化在顶层 `frontend_connections[]`，结构同样使用 `nodes[]` 与 `edges[]`：
...
GUI/frontend 用 `frontend_connections[] + pipelines[]` 恢复 File Input/File Output 节点及其连接；ConfigService 编译忽略 `frontend_connections[]`。
...
Build 成功并返回 `PIPE_LOADED` 后，GUI RUN 不再只切换 mock runtime state。前端 File Input 组件选择 WAV 时缓存浏览器 `File` 对象，并解析 RIFF `fmt ` 与 `data` chunk，只把 PCM data 区间用于运行期推送；工程 JSON 和 workspace snapshot 只保存文件名、path hint 与音频参数，不保存浏览器 File 对象。RUN 前 frontend 会校验 File Input/FILE_IO DAI source 已选择合法 PCM WAV，并校验 File Output 已选择 `.wav` 输出路径；校验失败则不会调用 `/api/runtime/run`。
...
8. SOF 基础 component 的 bytes 型参数由 `plugins/builtin_module_configs` 打包；只有声明 `PIPE_RUNNING` 的 bytes 参数才生成标准 `SectionControlBytes`。未知或 Audio Studio 专用参数继续进入 `AS_PRIVATE`，A2 ASoC Codec 驱动可从 private metadata 暴露自定义 KControl。
9. HOST/DAI node params 会写入 private block，包括 stream、link、device、`tdm_slots`、`slot_width`、`sample_bits`、`sample_rate` 等，用于 A2 codec/audio_controller 建立端口和 DAI 映射。
...
#### A.5 当前 JSON schema 摘要

当前平台 JSON 的核心 section：

| section | 说明 |
|---|---|
| `imports` | 导入共享 module catalog |
| `resource_catalog.compute` | DSP/core 资源描述 |
| `module_types` | 项目私有 module type，和 imported catalog 合并校验 |
| `pipelines` | 编译进 tplg 的 SOF graph；node 直接声明 `module_type` 与 `params` |
| `frontend_connections` | GUI/runtime File Input/Output 节点与 HOST external port 的连接，编译忽略 |
| `presets` | inspector/default/runtime 参数组 |

删除的冗余 section 不再兼容：独立实例表、pipeline 端点表、顶层命名模板和旧 frontend virtual 节点描述都不再解析。

HOST/DAI 作为普通 node 建模；其端点能力全部在 node `params` 中声明。三方算法扩展仍通过 `module_types[]`、`parameters[]` 和 module config plugin API 扩展，不受 HOST/DAI 特例影响。
