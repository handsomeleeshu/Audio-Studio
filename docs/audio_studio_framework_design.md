...
```text
server/drivers/core/include/audio_studio/drivers/core/driver_manager.hpp
...
server/drivers/filesystem/include/audio_studio/drivers/filesystem/filesystem_driver.hpp
server/drivers/filesystem/src/filesystem_driver.cpp
server/drivers/pipe/include/audio_studio/drivers/pipe/pipe_driver.hpp
...
server/drivers/audio/include/audio_studio/drivers/audio/audio_device.hpp
server/drivers/audio/src/audio_device.cpp
server/drivers/control/include/audio_studio/drivers/control/control_device.hpp
server/drivers/control/src/control_device.cpp
server/drivers/log/include/audio_studio/drivers/log/log_device.hpp
server/drivers/log/src/log_device.cpp
server/drivers/dump/include/audio_studio/drivers/dump/dump_device.hpp
server/drivers/dump/src/dump_device.cpp
server/drivers/dummy/include/audio_studio/drivers/dummy/dummy_driver.hpp
server/drivers/dummy/src/dummy_driver.cpp
```

`CONFIG_DRIVERS_CORE=y` 时构建 host-alone `DriverManager`，用于注册 driver category/name/detail 和 active 状态，先验证 driver registry/factory 边界。`CONFIG_DRIVER_OS=y` 时构建 deterministic OS driver，提供测试用 monotonic clock、env 和 systemInfo 能力，不创建真实线程或进程。`CONFIG_DRIVER_SOCKET=y` 时构建 loopback socket driver，覆盖 open/connect/send/receive 状态，不打开真实 TCP/UDP socket。`CONFIG_DRIVER_FILESYSTEM=y` 时构建 in-memory filesystem driver，覆盖 write/read/exists/list/joinPath，不访问真实磁盘。`CONFIG_DRIVER_PIPE=y` 时构建 in-memory pipe driver，覆盖 create/open/read/write/remove，不创建 POSIX FIFO 或 Windows named pipe。`CONFIG_DRIVER_DYNLIB=y` 时构建 mock dynlib driver，覆盖 open/registerSymbol/getSymbol 生命周期，不调用 dlopen/LoadLibrary。`CONFIG_DRIVER_TRANSPORT=y` 时构建 memory transport driver，覆盖 open/write/read/flush/caps，不绑定 socket、pipe 或平台 transport。`CONFIG_DRIVER_AUDIO=y` 时构建 host-alone audio device，覆盖 playback write 和 capture read 状态，不调用 ALSA/WASAPI/CoreAudio 或 Audio Controller。`CONFIG_DRIVER_CONTROL=y` 时构建 host-alone control device，覆盖 set/get/list/stats，不调用 tinymix 或 Audio Controller。`CONFIG_DRIVER_LOG=y` 时构建 host-alone log device，覆盖 raw chunk start/read/stats，不读取 firmware trace 或 `.ldc`。`CONFIG_DRIVER_DUMP=y` 时构建 host-alone dump device，覆盖 probe point add/list、packet start/read/stats，不解析 SOF probe stream 或写文件。`CONFIG_DRIVER_DUMMY=y` 时构建 dummy driver；它只在 host 内存中维护 open/start/stop/command/telemetry 状态，用于验证 server/framework 连接、CTest 和 CLI/RPC 后续接入，不访问真实设备。
