...
当前工程已经从早期 React 方案切换为 **standalone HTML 前端 + C++17 Mock Backend**。前端不需要 npm build；本地调试时由 C++ mock server 直接托管 `frontend/index.html`、静态资源和 `/api/*` 后端接口。
...
- JSON 解析相关逻辑在 `frontend/assets/js/configParser.js`。
