...
static void print_usage(void) {
  ...
  puts("");
  puts("This executable is the future Audio Studio peer/controller reference");
  puts("implementation. M0 intentionally only validates the top-level build");
  puts("layout before transport, protocol, and services are added.");
}
